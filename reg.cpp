...
//S*ST
Jac[nJac-1][nJac-1]=0; //производная по T
Nebal[nJac-1]=0;
for(i=0;i<(nJac-1)/2;i++)
...
/*
if(Raschtype==1&&Kn!=0)//для Кн
  DL+=(Rez[nJac-1]/Kn)*(Rez[nJac-1]/Kn);
else if(Raschtype==2&&T!=0)//для Т
...
for(i=0;i<n;i++)
 {
  if(mhu[i]==0||mhu[i]==4)
   ...
   {
    dU[i]=Rez[k]*alpha;//приращение для модуля напряжение, деленное на выч. коэфф-т amax (шаг метода Ньютона)
    if(Rez[k]>eps)
...
for(i=0;i<(nJac-1)/2;i++)
 {
  R[i]-=Rez[i+(nJac-1)/2]*alpha;//приращение для компонент собст. вектора
  if(Rez[i+(nJac-1)/2]>eps)
   RegOk=false;
 }
if(Raschtype==1)
 Kn-=Rez[nJac-1]*alpha;//приращение для Кн
else if(Raschtype==2)
 T-=Rez[nJac-1]*alpha;//приращение для Т
if(Rez[nJac-1]>eps)
 RegOk=false;
   ...
   if(i<GP->n_u)//dP/dU
    {
     k1=GP->Zp_r[GP->kG];
     i1=GP->Zu_r[i];
     if(k1==i1) //в диагональн. элементе в матр. якоби индексы у мощности и напряжия равны
      {
     ...
     else if((real(ys[k1][i1])==0&&imag(ys[k1][i1])==0)!=true)//в диагональн. элементе в матр. якоби индексы у мощности и напряжия НЕ СОВПАДАЮТ
      {
       if(GP->Zu[k1]!=-1)
        {
