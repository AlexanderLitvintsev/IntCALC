...
double amax=1;
if(LimitNebals==true)//если ограничиваем приращение переменных
{
k=0;
for(i=0;i<n;i++)
  if(mhu[i]!=0&&mhu[i]!=4)
   {
    if(amax<fabs(Rez[k])/(dUmax*abs(unom[i])))
      amax=fabs(Rez[k])/(dUmax*abs(unom[i])); //в случае превышения максимального приращения по данной переменной
    k++;
   }
k=0;
for(i=0;i<n;i++)
  if(mhu[i]!=0)
   {
    if(amax<fabs(Rez[k+n-nb-ng])/(dfmax*M_PI/180.))
      amax=fabs(Rez[k+n-nb-ng])/(dfmax*M_PI/180.);//в случае превышения максимального приращения по данной переменной
    k++;
   }

if(Raschtype!=0)//если считаем предельный режим (независимо Т или Кн)
if(NoKn!=-1)//если правильно задали компоненту вектора, которая заменяетяс на Т или Кн
{
for(i=0;i<nJac/2;i++)
 {
  if(NoKn!=i)//для компонент вектора
   {
   if(R[i]!=0)
    if(amax<fabs((R[i]-Rez[i+nJac/2])/R[i])/dVri)
     amax=fabs((R[i]-Rez[i+nJac/2])/R[i])/dVri;
   }
  else if(Raschtype==1)//для Кн
   {
   if(amax<fabs(Rez[i+nJac/2])/dKnmax)
     amax=fabs(Rez[i+nJac/2])/dKnmax;
   }
  else if(Raschtype==2)//для Т
   {
   if(amax<fabs((T-Rez[i+nJac/2])/T)/dTmax)
     amax=fabs((T-Rez[i+nJac/2])/T)/dTmax;
   }
 }
}
}
...
for(i=0;i<n;i++)
 {
  if(mhu[i]==0||mhu[i]==4)
    dU[i]=0;//для генераторов и б.у. приращение модуля напряжения=0
  else
   {
    dU[i]=Rez[k]/amax;//приращение для модуля напряжение, деленное на выч. коэфф-т amax (шаг метода Ньютона)
    if(Rez[k]>eps)
      RegOk=false;//режим не сошелся
    k++;
   }
 }
k=0;
for(i=0;i<n;i++)
 {
  if(mhu[i]==0)
    dD[i]=0;//для б.у. приращение фазы=0
  else
   {
    dD[i]=Rez[k+n-nb-ng]/amax;
...
for(i=0;i<nJac/2;i++)
 {
 if(NoKn!=i)
   R[i]-=Rez[i+nJac/2]/amax;//приращение для компонент собст. вектора
 else if(Raschtype==1)
   Kn-=Rez[i+nJac/2]/amax;//приращение для Кн
 else if(Raschtype==2)
   T-=Rez[i+nJac/2]/amax;//приращение для Т
 if(Rez[i+nJac/2]>eps)
   RegOk=false;
 }
}
...
k=0;
for(i=0;i<n;i++)
 urab[i]=polar(abs(urab[i])-dU[i],arg(urab[i])-dD[i]);//считаем напряжения в узлах в соотв. с фазами и модулями


for(i=0;i<nJac;i++)
 if(fabs(Nebal[i])>eps)//проверяем величину небалансов
  RegOk=false;
delete dU;
delete dD;
