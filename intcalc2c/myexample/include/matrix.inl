...
/* CVS $Id: matrix.inl,v 1.30 2010/12/21 09:54:01 cxsc Exp $ */
...
/*		int i,j,m,n;
		...
		int new_l1, new_u1, new_l2, new_u2; //start and end indices in ndat
			...
			for(int i=0 ; i<m1.ysize ; i++)
			  for(int k=0 ; k<m1.xsize ; k++)
			    for(int j=0 ; j<m2.xsize ; j++)
			      r.dat[i*m2.xsize+j] += m1.dat[i*m1.xsize+k] * m2.dat[k*m2.ysize+j];
			return r;
#endif
		} else {
...
#endif
	{
		...
		if(opdotprec == 1) {
			...
			for(int i=0 ; i<m1.ysize ; i++)
			  for(int k=0 ; k<m1.xsize ; k++)
			    for(int j=0 ; j<m2.xsize ; j++)
			      r.dat[i*m2.xsize+j] += fp_c_mult(m1.dat[i*m1.xsize+k], m2.dat[k*m2.ysize+j]);
			return r;
#endif
		} else {
