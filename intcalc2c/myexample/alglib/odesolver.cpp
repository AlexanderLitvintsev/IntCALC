...
bool odesolveriteration(odesolverstate& state)
{
    ...
    xc = state.xg(0);
    ap::vmove(&state.ytbl(0, 0), &state.yc(0), ap::vlen(0,n-1));
    for(j = 0; j <= n-1; j++)
    {
        state.escale(j) = 0;
    }
...
lbl_6:
    ...
    //
    ap::vmove(&state.yn(0), &state.yc(0), ap::vlen(0,n-1));
    ap::vmove(&state.yns(0), &state.yc(0), ap::vlen(0,n-1));
    k = 0;
lbl_8:
    if( k>5 )
    {
    ...
    ap::vmove(&state.y(0), &state.yc(0), ap::vlen(0,n-1));
    for(j = 0; j <= k-1; j++)
    {
        v = state.rkb(k,j);
        ap::vadd(&state.y(0), &state.rkk(j, 0), ap::vlen(0,n-1), v);
    }
...
lbl_0:
    ...
    v = h*state.xscale;
    ap::vmove(&state.rkk(k, 0), &state.dy(0), ap::vlen(0,n-1), v);
    
    //
    // update YN/YNS
    //
    v = state.rkc(k);
    ap::vadd(&state.yn(0), &state.rkk(k, 0), ap::vlen(0,n-1), v);
    v = state.rkcs(k);
    ap::vadd(&state.yns(0), &state.rkk(k, 0), ap::vlen(0,n-1), v);
    k = k+1;
    goto lbl_8;
lbl_10:
    ...
    //
    xc = xc+h;
    ap::vmove(&state.yc(0), &state.yn(0), ap::vlen(0,n-1));
...
lbl_7:
    ...
    // save result
    //
    ap::vmove(&state.ytbl(i, 0), &state.yc(0), ap::vlen(0,n-1));
    i = i+1;
    goto lbl_3;
lbl_5:
     ...
     odesolverreport& rep)
{
    ...
    if( rep.terminationtype>0 )
    {
        ...
        v = state.xscale;
        ap::vmove(&xtbl(0), &state.xg(0), ap::vlen(0,state.m-1), v);
        ytbl.setlength(state.m, state.n);
        for(i = 0; i <= state.m-1; i++)
        {
            ap::vmove(&ytbl(i, 0), &state.ytbl(i, 0), ap::vlen(0,state.n-1));
        }
    }
    else
    {
     ...
     odesolverstate& state)
{
    ...
    if( m==1 )
    {
        ...
        ap::vmove(&state.ytbl(0, 0), &y(0), ap::vlen(0,n-1));
        state.xg.setlength(m);
        ap::vmove(&state.xg(0), &x(0), ap::vlen(0,m-1));
        return;
    }
    ...
    {
        state.xscale = -1;
        ap::vmul(&state.xg(0), ap::vlen(0,m-1), -1);
    }
    state.yc.setlength(n);
    ap::vmove(&state.yc(0), &y(0), ap::vlen(0,n-1));
