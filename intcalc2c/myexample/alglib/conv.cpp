     ...
     ap::real_1d_array& r)
{
    ...
    buf.setlength(p);
    ap::vmove(&buf(0), &a(0), ap::vlen(0,m-1));
    for(i = m; i <= p-1; i++)
    {
        buf(i) = 0;
    }
    buf2.setlength(p);
    ap::vmove(&buf2(0), &b(0), ap::vlen(0,n-1));
    for(i = n; i <= p-1; i++)
    {
        buf2(i) = 0;
    }
    ...
    r.setlength(m-n+1);
    ap::vmove(&r(0), &buf(0), ap::vlen(0,m-n));
}
     ...
     ap::real_1d_array& c)
{
    ...
    if( m<n )
    {
        ...
        while(i1<n)
        {
            i2 = ap::minint(i1+m-1, n-1);
            j2 = i2-i1;
            ap::vadd(&buf(0), &r(i1), ap::vlen(0,j2));
            i1 = i1+m;
        }
     ...
     ap::real_1d_array& r)
{
    ...
    if( m<n )
    {
        ...
        while(i1<n)
        {
            i2 = ap::minint(i1+m-1, n-1);
            j2 = i2-i1;
            ap::vadd(&buf(0), &b(i1), ap::vlen(0,j2));
            i1 = i1+m;
        }
     ...
     ap::real_1d_array& r)
{
    ...
    if( alg==0 )
    {
        ...
        if( n==1 )
        {
            r.setlength(m);
            v = b(0);
            ap::vmove(&r(0), &a(0), ap::vlen(0,m-1), v);
            return;
        }
        ...
        if( circular )
        {
            ...
            for(i = 1; i <= n-1; i++)
            {
                ...
                j1 = m-i;
                j2 = m-1;
                ap::vadd(&r(i1), &a(j1), ap::vlen(i1,i2), v);
                i1 = i;
                i2 = m-1;
                j1 = 0;
                j2 = m-i-1;
        ...
        else
        {
            ...
            {
                v = b(i);
                ap::vadd(&r(i), &a(0), ap::vlen(i,i+m-1), v);
            }
        }
        return;
    }
    ...
    if( alg==1 )
    {
        ...
        else
        {
            ...
            if( circular )
            {
                ...
                if( n>=2 )
                {
                    ap::vadd(&r(0), &buf(m), ap::vlen(0,n-2));
                }
            }
            else
            {
                ...
                //
                r.setlength(m+n-1);
                ap::vmove(&r(0), &buf(0), ap::vlen(0,m+n-2));
            }
        }
        return;
    }
    ...
    if( alg==2 )
    {
        ...
        {
            p = ap::minint(q, m-i);
            ap::vmove(&buf(0), &a(i), ap::vlen(0,p-1));
            for(j = p; j <= q+n-2; j++)
            {
                buf(j) = 0;
            }
            ...
            ap::vadd(&r(i), &buf(0), ap::vlen(i,i+j1));
            if( p+n-2>=j2 )
            {
                ap::vadd(&r(0), &buf(j2), ap::vlen(0,p+n-2-j2));
            }
            i = i+p;
        }
        return;
