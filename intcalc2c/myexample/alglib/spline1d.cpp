...
void spline1dcopy(const spline1dinterpolant& c, spline1dinterpolant& cc)
{
    ...
    cc.k = c.k;
    cc.x.setlength(cc.n);
    ap::vmove(&cc.x(0), &c.x(0), ap::vlen(0,cc.n-1));
    cc.c.setlength((cc.k+1)*(cc.n-1));
    ap::vmove(&cc.c(0), &c.c(0), ap::vlen(0,(cc.k+1)*(cc.n-1)-1));
}
     ...
     int& ralen)
{
    ...
    ra(2) = c.n;
    ra(3) = c.k;
    ap::vmove(&ra(4), &c.x(0), ap::vlen(4,4+c.n-1));
    ap::vmove(&ra(4+c.n), &c.c(0), ap::vlen(4+c.n,4+c.n+(c.k+1)*(c.n-1)-1));
}
...
void spline1dunserialize(const ap::real_1d_array& ra, spline1dinterpolant& c)
{
    ...
    ap::vmove(&c.x(0), &ra(4), ap::vlen(0,c.n-1));
    ap::vmove(&c.c(0), &ra(4+c.n), ap::vlen(0,(c.k+1)*(c.n-1)-1));
}
     ...
     spline1dfitreport& rep)
{
    ...
    if( st==0 )
    {
        ...
        // cubic spline basis
        //
        ap::vmove(&sy(0), &tmp(0), ap::vlen(0,m-2-1));
