    ...
    if( isupper )
    {
        ...
        while(k<=n-1)
        {
            if( pivots(k)>=0 )
            {
                ...
                if( k>0 )
                {
                    ap::vmove(work.getvector(1, k), a.getcolumn(k, 0, k-1));
                    symmetricmatrixvectormultiply(a, isupper, 1-1, k+1-1-1, work, double(-1), work2);
                    ap::vmove(a.getcolumn(k, 0, k-1), work2.getvector(1, k));
                    v = ap::vdotproduct(&work2(1), &work(1), ap::vlen(1,k));
                    a(k,k) = a(k,k)-v;
                }
                kstep = 1;
            }
            else
            {
                ...
                if( k>0 )
                {
                    ...
                    ap::vmove(a.getcolumn(k, 0, k-1), work2.getvector(1, k));
                    v = ap::vdotproduct(&work(1), &work2(1), ap::vlen(1,k));
                    a(k,k) = a(k,k)-v;
                    v = ap::vdotproduct(a.getcolumn(k, 0, k-1), a.getcolumn(k+1, 0, k-1));
                    a(k,k+1) = a(k,k+1)-v;
                    ap::vmove(work.getvector(1, k), a.getcolumn(k+1, 0, k-1));
                    symmetricmatrixvectormultiply(a, isupper, 0, k-1, work, double(-1), work2);
                    ap::vmove(a.getcolumn(k+1, 0, k-1), work2.getvector(1, k));
                    v = ap::vdotproduct(&work(1), &work2(1), ap::vlen(1,k));
            ...
            if( kp!=k )
            {
                ...
                // submatrix
                //
                ap::vmove(work.getvector(1, kp), a.getcolumn(k, 0, kp-1));
                ap::vmove(a.getcolumn(k, 0, kp-1), a.getcolumn(kp, 0, kp-1));
                ap::vmove(a.getcolumn(kp, 0, kp-1), work.getvector(1, kp));
                ap::vmove(work.getvector(1, k-1-kp), a.getcolumn(k, kp+1, k-1));
                ap::vmove(a.getcolumn(k, kp+1, k-1), a.getrow(kp, kp+1, k-1));
                ap::vmove(&a(kp, kp+1), &work(1), ap::vlen(kp+1,k-1));
                temp = a(k,k);
                a(k,k) = a(kp,kp);
                a(kp,kp) = temp;
                if( kstep==2 )
    ...
    else
    {
        ...
        while(k>=0)
        {
            if( pivots(k)>=0 )
            {
                ...
                if( k<n-1 )
                {
                    ap::vmove(work.getvector(1, n-k-1), a.getcolumn(k, k+1, n-1));
                    symmetricmatrixvectormultiply(a, isupper, k+1, n-1, work, double(-1), work2);
                    ap::vmove(a.getcolumn(k, k+1, n-1), work2.getvector(1, n-k-1));
                    v = ap::vdotproduct(&work(1), &work2(1), ap::vlen(1,n-k-1));
                    a(k,k) = a(k,k)-v;
                }
                kstep = 1;
            }
            else
            {
                ...
                if( k<n-1 )
                {
                    ...
                    v = ap::vdotproduct(&work(1), &work2(1), ap::vlen(1,n-k-1));
                    a(k,k) = a(k,k)-v;
                    v = ap::vdotproduct(a.getcolumn(k, k+1, n-1), a.getcolumn(k-1, k+1, n-1));
                    a(k,k-1) = a(k,k-1)-v;
                    ap::vmove(work.getvector(1, n-k-1), a.getcolumn(k-1, k+1, n-1));
                    symmetricmatrixvectormultiply(a, isupper, k+1, n-1, work, double(-1), work2);
                    ap::vmove(a.getcolumn(k-1, k+1, n-1), work2.getvector(1, n-k-1));
                    v = ap::vdotproduct(&work(1), &work2(1), ap::vlen(1,n-k-1));
                    a(k-1,k-1) = a(k-1,k-1)-v;
                }
                kstep = 2;
            ...
            if( kp!=k )
            {
                ...
                if( kp<n-1 )
                {
                    ap::vmove(work.getvector(1, n-kp-1), a.getcolumn(k, kp+1, n-1));
                    ap::vmove(a.getcolumn(k, kp+1, n-1), a.getcolumn(kp, kp+1, n-1));
                    ap::vmove(a.getcolumn(kp, kp+1, n-1), work.getvector(1, n-kp-1));
                }
                ap::vmove(work.getvector(1, kp-k-1), a.getcolumn(k, k+1, kp-1));
                ap::vmove(a.getcolumn(k, k+1, kp-1), a.getrow(kp, k+1, kp-1));
                ap::vmove(&a(kp, k+1), &work(1), ap::vlen(k+1,kp-1));
                temp = a(k,k);
                a(k,k) = a(kp,kp);
                a(kp,kp) = temp;
                if( kstep==2 )
        ...
        while(k<=n)
        {
            if( pivots(k)>0 )
            {
                ...
                if( k>1 )
                {
                    ...
                    v = ap::vdotproduct(&work2(1), &work(1), ap::vlen(1,km1));
                    a(k,k) = a(k,k)-v;
                }
                kstep = 1;
            }
            else
            {
                ...
                if( k>1 )
                {
                    ...
                    symmetricmatrixvectormultiply(a, isupper, 1, k-1, work, double(-1), work2);
                    ap::vmove(a.getcolumn(k, 1, km1), work2.getvector(1, km1));
                    v = ap::vdotproduct(&work(1), &work2(1), ap::vlen(1,km1));
                    a(k,k) = a(k,k)-v;
            ...
            if( kp!=k )
            {
                ...
                ap::vmove(&a(kp, i1), &work(1), ap::vlen(i1,i2));
                temp = a(k,k);
                a(k,k) = a(kp,kp);
                a(kp,kp) = temp;
                if( kstep==2 )
        ...
        while(k>=1)
        {
            if( pivots(k)>0 )
            {
                ...
                if( k<n )
                {
                    ...
                    v = ap::vdotproduct(&work(1), &work2(1), ap::vlen(1,l));
                    a(k,k) = a(k,k)-v;
                }
                kstep = 1;
            }
