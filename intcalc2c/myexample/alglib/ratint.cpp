...
void barycentriclintransy(barycentricinterpolant& b, double ca, double cb)
{
    ...
    {
        v = 1/b.sy;
        ap::vmul(&b.y(0), ap::vlen(0,b.n-1), v);
    }
}
     ...
     ap::real_1d_array& w)
{
    ...
    w.setlength(n);
    v = b.sy;
    ap::vmove(&x(0), &b.x(0), ap::vlen(0,n-1));
    ap::vmove(&y(0), &b.y(0), ap::vlen(0,n-1), v);
    ap::vmove(&w(0), &b.w(0), ap::vlen(0,n-1));
}
     ...
     int& ralen)
{
    ...
    ra(2) = b.n;
    ra(3) = b.sy;
    ap::vmove(&ra(4), &b.x(0), ap::vlen(4,4+b.n-1));
    ap::vmove(&ra(4+b.n), &b.y(0), ap::vlen(4+b.n,4+2*b.n-1));
    ap::vmove(&ra(4+2*b.n), &b.w(0), ap::vlen(4+2*b.n,4+3*b.n-1));
}
     ...
     barycentricinterpolant& b)
{
    ...
    b.y.setlength(b.n);
    b.w.setlength(b.n);
    ap::vmove(&b.x(0), &ra(4), ap::vlen(0,b.n-1));
    ap::vmove(&b.y(0), &ra(4+b.n), ap::vlen(0,b.n-1));
    ap::vmove(&b.w(0), &ra(4+2*b.n), ap::vlen(0,b.n-1));
}
     ...
     barycentricinterpolant& b2)
{
    ...
    b2.y.setlength(b2.n);
    b2.w.setlength(b2.n);
    ap::vmove(&b2.x(0), &b.x(0), ap::vlen(0,b2.n-1));
    ap::vmove(&b2.y(0), &b.y(0), ap::vlen(0,b2.n-1));
    ap::vmove(&b2.w(0), &b.w(0), ap::vlen(0,b2.n-1));
}
     ...
     barycentricinterpolant& b)
{
    ...
    b.y.setlength(n);
    b.w.setlength(n);
    ap::vmove(&b.x(0), &x(0), ap::vlen(0,n-1));
    ap::vmove(&b.y(0), &y(0), ap::vlen(0,n-1));
    ap::vmove(&b.w(0), &w(0), ap::vlen(0,n-1));
    b.n = n;
     ...
     barycentricfitreport& rep)
{
    ...
    for(d = 0; d <= ap::minint(9, n-1); d++)
    {
        ...
        if( locinfo>0 )
        {
            ...
            if( ap::fp_less(wrmscur,wrmsbest)||ap::fp_less(rep.dbest,0) )
            {
...
static void barycentricnormalize(barycentricinterpolant& b)
{
    ...
    {
        v = 1/v;
        ap::vmul(&b.w(0), ap::vlen(0,b.n-1), v);
    }
    for(i = 0; i <= b.n-2; i++)
    {
     ...
     barycentricfitreport& rep)
{
    ...
    for(i = 0; i <= n-1; i++)
    {
        ...
        ap::vmove(&fmatrix(i, 0), &sbf(0), ap::vlen(0,m-1));
        y2(i) = y(i);
        w2(i) = w(i);
        mx = mx+fabs(w(i))/n;
    }
    ...
    //
    ap::vmove(&sy(0), &tmp(0), ap::vlen(0,m-1));
