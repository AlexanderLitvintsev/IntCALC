     ...
     lsfitreport& rep)
{
    ...
    else
    {
        ...
        for(i = 0; i <= k-1; i++)
        {
            if( i>0 )
            {
                v = ap::vdotproduct(&cmatrix(i, 0), &tmp(0), ap::vlen(0,i-1));
            }
            else
            {
                v = 0;
        ...
        {
            v = tmp(i);
            ap::vadd(&c0(0), &q(i, 0), ap::vlen(0,m-1), v);
        }
        ...
        //
        c.setlength(m);
        ap::vmove(&c(0), &c0(0), ap::vlen(0,m-1));
     ...
     lsfitstate& state)
{
    ...
    if( n>=1&&m>=1&&k>=1 )
    {
        ...
        state.w.setlength(n);
        state.c.setlength(k);
        ap::vmove(&state.c(0), &c(0), ap::vlen(0,k-1));
        ap::vmove(&state.w(0), &w(0), ap::vlen(0,n-1));
        for(i = 0; i <= n-1; i++)
        {
            ap::vmove(&state.taskx(i, 0), &x(i, 0), ap::vlen(0,m-1));
            state.tasky(i) = y(i);
        }
    }
...
bool lsfitnonlineariteration(lsfitstate& state)
{
    ...
    if( state.havehess )
    {
        ...
        //
        minlmfgh(k, state.c, ap::sqr(state.epsf)*n, state.epsx, state.maxits, state.optstate);
    }
    else
    {
        ...
        if( state.cheapfg )
        {
            minlmfgj(k, n, state.c, ap::sqr(state.epsf)*n, state.epsx, state.maxits, state.optstate);
        }
        else
        {
            minlmfj(k, n, state.c, ap::sqr(state.epsf)*n, state.epsx, state.maxits, state.optstate);
        }
    }
...
lbl_9:
    ...
    ap::vmove(&state.c(0), &state.optstate.x(0), ap::vlen(0,k-1));
    ap::vmove(&state.x(0), &state.taskx(i, 0), ap::vlen(0,m-1));
    state.pointindex = i;
...
lbl_1:
    ...
    ap::vadd(&state.optstate.g(0), &state.g(0), ap::vlen(0,k-1), v);
    i = i+1;
    goto lbl_14;
lbl_16:
    goto lbl_5;
...
lbl_2:
    ...
    v = state.w(i);
    ap::vmove(&state.optstate.j(i, 0), &state.g(0), ap::vlen(0,k-1), v);
    i = i+1;
    goto lbl_19;
lbl_21:
    goto lbl_5;
...
lbl_3:
    ...
    for(j = 0; j <= k-1; j++)
    {
        ...
        ap::vadd(&state.optstate.h(j, 0), &state.g(0), ap::vlen(0,k-1), v);
        v = 2*ap::sqr(state.w(i))*(state.f-state.tasky(i));
        ap::vadd(&state.optstate.h(j, 0), &state.h(j, 0), ap::vlen(0,k-1), v);
    }
    i = i+1;
    goto lbl_24;
lbl_26:
...
lbl_29:
    ...
    ap::vmove(&state.c(0), &state.c(0), ap::vlen(0,k-1));
    ap::vmove(&state.x(0), &state.taskx(i, 0), ap::vlen(0,m-1));
    state.pointindex = i;
    lsfitclearrequestfields(state);
    state.needf = true;
     ...
     lsfitreport& rep)
{
    ...
    {
        c.setlength(state.k);
        ap::vmove(&c(0), &state.c(0), ap::vlen(0,state.k-1));
     ...
     ap::real_1d_array& yoriginal)
{
    ...
    ap::vmove(&xoriginal(0), &x(0), ap::vlen(0,n-1));
    xa = xmin;
    xb = xmax;
    for(i = 0; i <= n-1; i++)
    {
    ...
    ap::vmove(&yoriginal(0), &y(0), ap::vlen(0,n-1));
    sa = 0;
    for(i = 0; i <= n-1; i++)
    {
        sa = sa+y(i);
     ...
     lsfitreport& rep)
{
    ...
    if( n<m )
    {
        ...
        {
            v = w(j);
            ap::vmove(&ft(j, 0), &fmatrix(j, 0), ap::vlen(0,m-1), v);
            b(j) = w(j)*y(j);
            wmod(j) = 1;
        }
        ...
        {
            v = tmp(i);
            ap::vadd(&c(0), &q(i, 0), ap::vlen(0,m-1), v);
        }
        return;
    }
    ...
    {
        v = b(i);
        ap::vadd(&tmp(0), &q(i, 0), ap::vlen(0,m-1), v);
    }
    b.setlength(m);
    ap::vmove(&b(0), &tmp(0), ap::vlen(0,m-1));
    ...
    if( ap::fp_greater(rep.taskrcond,threshold) )
    {
        ...
        for(i = m-2; i >= 0; i--)
        {
            v = ap::vdotproduct(&r(i, i+1), &c(i+1), ap::vlen(i+1,m-1));
            c(i) = (b(i)-v)/r(i,i);
        }
    }
    else
    {
        ...
        {
            v = b(i);
            ap::vadd(&utb(0), &u(i, 0), ap::vlen(0,m-1), v);
        }
        ...
        {
            v = sutb(i);
            ap::vadd(&c(0), &vt(i, 0), ap::vlen(0,m-1), v);
        }
    }
