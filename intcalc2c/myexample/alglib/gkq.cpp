     ...
     ap::real_1d_array& wgauss)
{
    ...
    ap::vmove(&ta(0), &alpha(0), ap::vlen(0,ap::ifloor(double(3*n)/double(2))));
    alpha.setlength(2*n+1);
    ap::vmove(&alpha(0), &ta(0), ap::vlen(0,ap::ifloor(double(3*n)/double(2))));
    for(i = ap::ifloor(double(3*n)/double(2))+1; i <= 2*n; i++)
    {
        alpha(i) = 0;
    }
    ta.setlength(ap::iceil(double(3*n)/double(2))+1);
    ap::vmove(&ta(0), &beta(0), ap::vlen(0,ap::iceil(double(3*n)/double(2))));
    beta.setlength(2*n+1);
    ap::vmove(&beta(0), &ta(0), ap::vlen(0,ap::iceil(double(3*n)/double(2))));
    ...
    for(m = 0; m <= n-2; m++)
    {
        ...
        ap::vmove(&ta(0), &t(0), ap::vlen(0,wlen-1));
        ap::vmove(&t(0), &s(0), ap::vlen(0,wlen-1));
        ap::vmove(&s(0), &ta(0), ap::vlen(0,wlen-1));
    }
    for(j = n/2; j >= 0; j--)
    {
