...
//      Form1->ImageForRightBtn->ParamNames[i]=ParamNames[i];
...
else
  {
   ...
   //   ShowMessage("Введены ошибочные параметры");
  }
}
//---------------------------------------------------------------------------
...
if(ElementName!="Шина"&&ElementName!="Система"&&ElementName!="Нагрузка")
  ShowDB=true; //показывать кнопку "Справочник"
else
  ShowDB=false;


if(ElementName.UpperCase().Pos("ТРАНСФОРМАТОР")>0)
  UseParamsCheck=true; //показывать галку для альтернативных параметров
else
...
//    Label[i]->Width=130;
    ...
    if(UseParamsCheck==true&&ParamNames[i]=="n, шт")
      {
       CurrPanel=Panel2;
...
Form1->OpenDialog1->Filter="Текстовый файл с параметрами (*.txt)| *.txt";
...
if(Form1->OpenDialog1->FileName!="")
...
else
  {
   ParFileName="";
   ParFromFile=false;
   Edit[ParamsNumber]->Text="не выбран";
